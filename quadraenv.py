...
class QuadraSetEnv(object):
    def __init__(self):
        ...
        self.allclients = []
        self.clientsactuel = []
        self.liste_dossiers_interdits = ["000101", "016645",
        "016645", "000206","000100","000500","000600", "000700",
        "000922", "000923","000924", "000925","000926", "000947", 
        "000272", "N00101", "X00101"]
        self.ipl = r"\\srvquadra\qappli\quadra\database\client\quadra.ipl"
        self.read_ipl(self.ipl)
    ...
    def read_ipl(self, ipl_file):
        ...
        for item in list(self.cur):
            code, nom = item
            self.allclients.append([code , nom])
            
        for (numclt , nomclt) in self.allclients:
            if nomclt != "" and  numclt not in self.liste_dossiers_interdits:
                self.clientsactuel.append([numclt , nomclt])

        # Build list client actuel
    ...
    # methode a améliorer afin de prendre en compte tout les contextes d'utilisation possible.
    def Millesime(self , numclt):
        '''
        Retourn un dictionnaire contenant le ou les millesimes du client + le path pour y accéder.
        '''
        self.cpta_path = r"\\srvquadra\Qappli\Quadra\DATABASE\cpta"
        self.client = numclt
        self.dir = os.listdir(self.cpta_path)
        now = datetime.now().year  # l'année actuelle
        millesime = []
        # Nous devons réduire le listing des dossiers a annalyser.
        # Pour ce faire : Nous devons éliminer les dossier DA et n'afficher que les dossiers DS<ou=à n-1
        #               : 
        for folder in self.dir:
            
            if folder[:6].upper() in ["DS"+str(now), 'DS'+str(now -1), "DC" ]:
            #if 'DS'+str(now) in folder.upper() or 'DS'+str(now -1)in folder.upper() or 'DC' in folder:
                path = os.path.join(self.cpta_path,folder,numclt,"qcompta.mdb")
                if os.path.isfile(path):

                    millesime.append((folder , path))
        return millesime
